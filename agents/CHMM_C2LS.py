...
class CHMM_C2LS:
    ...
    def compute_efe_loss(self, config, obs, actions, next_obs, done, rewards):
        ...
        if config["enable_tensorboard"] and self.steps_done % 10 == 0:
            self.writer.add_scalar("gval max", gval.max(), self.steps_done)
            self.writer.add_scalar("gval mean", gval.mean(), self.steps_done)
            self.writer.add_scalar("gval min", gval.min(), self.steps_done)
